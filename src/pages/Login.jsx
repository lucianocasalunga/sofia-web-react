...
export default function Login() {
  ...
  const [tab, setTab]         = useState('extension') // 'extension' | 'nsec'
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="bg-slate-900/80 backdrop-blur-xl rounded-2xl border border-slate-800 p-6 shadow-2xl">
          ...
          <div className="flex gap-2 mb-5 bg-slate-800/50 rounded-xl p-1">
            {[['extension', '🔌 Extensão'], ['nsec', '🔑 Chave Privada']].map(([key, label]) => (
              <button key={key}
                onClick={() => { setTab(key); setError('') }}
                className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${
                  tab === key ? 'bg-emerald-500 text-black' : 'text-slate-400 hover:text-white'
                }`}
              >{label}</button>
            ))}
          </div>
          ...
          {tab === 'extension' ? (
            <div className="space-y-3">
              ...
              >
                ...
                {loading ? 'Conectando...' : 'Entrar com Extensão Nostr'}
              </button>
              {!hasExtension && (
                <p className="text-center text-xs text-slate-500">
                  Instale <a href="https://getalby.com" target="_blank" rel="noreferrer" className="text-emerald-400 underline">Alby</a> ou{' '}
                  <a href="https://github.com/fiatjaf/nos2x" target="_blank" rel="noreferrer" className="text-emerald-400 underline">nos2x</a> para usar este método
                </p>
              )}
            </div>
          ) : (
